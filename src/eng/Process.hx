...
class Process {
	...
	inline function get_fx() return App.inst.fx;
	public var world(get, never):World;
	inline function get_world() return App.inst.world;
	public var hud(get, never):ui.Hud;
	inline function get_hud() return App.inst.hud;
	public var camera(get, never):Camera;
	inline function get_camera() return App.inst.camera;
	public var input(get, never):Input;
	inline function get_input() return App.inst.input;
	public var events(get, never):EventRouter;
	inline function get_events() return App.inst.events;
	public var console(get, never):Console;
	inline function get_console() return App.inst.console;
	public var audio(get, never):Audio;
	inline function get_audio() return App.inst.audio;
	public var util(get, never):Util;
	inline function get_util() return App.inst.util;
	...
	inline function get_tmod() return utmod * getComputedTimeMultiplier();
	...
	public function new(?parent:Process) {
		...
		if(parent == null) ROOTS.push(this); else parent.addChild(this);
	}
	...
	public function createRoot(?ctx:h2d.Object) {
		...
		if(ctx == null) {
			if(parent == null || parent.root == null) throw this + ": context required";
			ctx = parent.root;
		}
	...
	public function createRootInLayers(ctx:h2d.Layers, plan:Int) {
		if(root != null) throw this + ": root already exists";
		root = new h2d.Layers();
		root.name = getDisplayName();
		ctx.add(root, plan);
	}
	...
	public function rprintChildren():String {
		function _crawl(p:Process, depth = 0) {
			...
			for(cp in p.children) out = out.concat(_crawl(cp, depth + 1));
			return out;
		}
	...
	public static function rprintAll():String {
		var all = [];
		for(p in ROOTS) all.push(p.rprintChildren());
		return all.join("\n");
	}
	...
	**/
	...
	public inline function w() {
		if(CUSTOM_STAGE_WIDTH > 0) return CUSTOM_STAGE_WIDTH;
	...
	public inline function h() {
		if(CUSTOM_STAGE_HEIGHT > 0) return CUSTOM_STAGE_HEIGHT;
	...
	public inline function isPaused() {
		if(_manuallyPaused) return true; else return anyParentPaused();
	}
	...
	public function createChildProcess(?onUpdate:Process->Void, ?onDispose:Process->Void, ?runUpdateImmediatly = false):Process {
		...
		if(onUpdate != null) p.onUpdateCb = function() {
			onUpdate(p);
		}

		if(onDispose != null) p.onDisposeCb = function() {
			onDispose(p);
		}
	...
	static inline function _doPreUpdate(p:Process, utmod:Float) {
		...
		if(canRun(p)) p.udelayer.update(p.utmod);

		if(canRun(p)) p.cd.update(p.tmod);

		if(canRun(p)) p.ucd.update(p.utmod);

		if(canRun(p)) p.tw.update(p.tmod);
		...
		if(canRun(p))for(c in p.children) _doPreUpdate(c, p.utmod);
	}
	...
	static function _doMainUpdate(p:Process) {
		...
		if(canRun(p))for(p in p.children) _doMainUpdate(p);
	}
	...
	static function _doFixedUpdate(p:Process) {
		...
		if(canRun(p))for(p in p.children) _doFixedUpdate(p);
	}
	...
	static inline function _doPostUpdate(p:Process) {
		...
		if(!p.destroyed)for(c in p.children) _doPostUpdate(c);
	}
	...
	static function _garbageCollector(plist:Array<Process>) {
		...
		while(i < plist.length) {
			var p = plist[i];
			if(p.destroyed) _disposeProcess(p); else {
	...
	static inline function _resizeProcess(p:Process) {
		if(!p.destroyed) {
			p.onResize();
			for(p in p.children) _resizeProcess(p);
		}
	}
	...
	public static function updateAll(utmod:Float) {
		for(p in ROOTS) _doPreUpdate(p, utmod);

		for(p in ROOTS) _doMainUpdate(p);

		for(p in ROOTS) _doFixedUpdate(p);

		for(p in ROOTS) _doPostUpdate(p);

		_garbageCollector(ROOTS);
	}

	public static function resizeAll() {
		for(p in ROOTS) _resizeProcess(p);
	}
