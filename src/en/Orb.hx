...
class Orb extends Entity {
	...
	public function generateEnergy() {
		energyOrbs = [
			for(i in 0...energy) {
				pos: Vector2.from_radians(M.frandRange(0, 2 * Math.PI), M.frandRange(-radius, radius)),
				vel: Vector2.from_radians(M.frandRange(0, 2 * Math.PI), (1 / radius) * 30),
				destroyed: false,
				timestamp: 0.0,
			}
		];
		...
		if(camera.entityOnScreen(this, 200)) {
			...
			for(e in energyOrbs) {
				...
				if(e.pos.length >= radius - 5) {
					e.vel = Vector2.from_radians(M.frandRange(0, 2 * Math.PI), (1 / radius) * 30);
				}
